module "ingress_alb" {
  ...
  internal = false
  name    = "${local.resource_name}-ingress_alb" #expense-dev-app-alb
  vpc_id  = local.vpc_id
...
module "records" {
 ...
 resource "aws_lb_target_group" "expense" {
  ...
  condition {
    host_header {
      values = ["expense-${var.enivronment}.${var.zone_name}"] #expense-dev.daws81s.online
    }
  }
}